...
class Users extends Admin_Controller 
{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
                ...
                $data1 = array(
                    ...
                    'user_id' => $user_id,
                    'action_made' => 'Created ['.$this->input->post('username').'] in the User list',
                );
		        ...
		        if(empty($this->input->post('password')) && empty($this->input->post('cpassword'))) {
		        	...
		        	if($update == true) {
						...
						$data1 = array(
							'date_time' => strtotime(date('Y-m-d h:i:s a')),
							'user_id' => $user_id,
							'action_made' => 'Updated ['.$this->input->post('username').'] in the User list',
						);
	...
	public function delete($id)
	{
		...
		// Check if the user account is currently in use
		if($this->model_users->checkUserInUse($id)) {
			$this->session->set_flashdata('error', 'This user is currently in use and cannot be deleted.');
			redirect('users');
		}
		if($id) {
				...
				if($delete == true) {
					...
					$user_id = $this->session->userdata('id');
						...
						$data1 = array(
							'date_time' => strtotime(date('Y-m-d h:i:s a')),
							'user_id' => $user_id,
							'action_made' => 'Deleted [id = '.$id.'] in the User list',
						);
		        ...
		        if(empty($this->input->post('password')) && empty($this->input->post('cpassword'))) {
		        	...
		        	if($update == true) {
						...
						$data1 = array(
							'date_time' => strtotime(date('Y-m-d h:i:s a')),
							'user_id' => $user_id,
							'action_made' => 'Updated User Information',
						);
